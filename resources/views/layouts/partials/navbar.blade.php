        ...
        <div :class="{'block': mobileMenuOpen, 'hidden': !mobileMenuOpen}" class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
            <ul class="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-700 rounded-lg md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0">
                ...
                <li x-data="{ dropdownOpen: false }" class="relative">
                    ...
                    <div x-show="dropdownOpen" x-transition class="absolute mt-2 w-48 bg-white rounded-lg shadow-lg z-10">
                        {{-- Loop data dari View Composer --}}
                        @forelse ($profilPages as $page)
                            <a href="{{ route('halaman.show', $page->slug) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">{{ $page->title }}</a>
                        @empty
